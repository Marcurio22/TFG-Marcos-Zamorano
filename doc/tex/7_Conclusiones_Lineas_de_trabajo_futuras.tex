...
\begin{itemize}
	...
	\item Se ha diseñado un sistema extensible para la gestión de modelos, evitando que la aplicación dependa de un único artefacto fijo. Los administradores pueden incorporar modelos, validarlos antes de su uso y seleccionar cuál debe actuar como modelo activo, facilitando así la evolución futura del sistema.
	
	\item Ha sido necesario estudiar en profundidad tanto el artículo base~\cite{diezpastora2025trails} como el algoritmo proporcionado, analizando el flujo de preparación de imágenes, carga de modelos, ejecución de predicciones y generación de máscaras antes de adaptarlo a una aplicación con interacción real de usuario.
...
A partir del trabajo realizado, las líneas de mejora halladas son:
...
\begin{itemize}
	...
	\item Desde el punto de vista algorítmico, sería conveniente incorporar un módulo de evaluación asistida que permitiese comparar las trazas generadas con anotaciones manuales de referencia. Esto permitiría calcular métricas como \textit{IoU}, \textit{Dice}, precisión, \textit{recall} o $F_1$ sobre imágenes concretas, facilitando la validación de nuevos modelos y el análisis cuantitativo de su rendimiento en escenarios reales.
	
	\item Finalmente, se podría evolucionar el sistema hacia una arquitectura más robusta para procesamiento intensivo, sustituyendo el \textit{worker} ligero actual por una cola de tareas más especializada. Herramientas de este tipo permitirían gestionar mejor trabajos concurrentes, reintentos, prioridades, cancelaciones y monitorización, haciendo que la aplicación fuese más adecuada para escenarios con mayor número de usuarios o colecciones de gran tamaño.
\end{itemize}
