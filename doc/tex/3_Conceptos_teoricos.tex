...
\begin{figure}[H]
	...
	\caption[Ejemplo de segmentación semántica.]{Ejemplo de segmentación semántica: (a) imagen original y (b) máscara segmentada. Fuente: MartinThoma. Licencia: CC0 1.0.}
...
\subsection{Estructuras ultra-profundas}
...
\subsection{Predicción Densa (\textit{Dense prediction})}
...
\begin{enumerate}
	\item Codificación mediante encoder:
...
\section{No linealidades y capa de decisión}
...
\section{Consideraciones de generalización y propiedades espaciales}
...
\subsection{Invarianza traslacional}
...
\subsection{Equivarianza traslacional}
...
\section{Arquitecturas de Segmentación Semántica}
...
\section{Evaluación experimental de la segmentación semántica}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\end{scope}
	\end{tikzpicture}
	\caption[Interpretación geométrica de IoU.]{Interpretación geométrica de IoU como solapamiento entre la máscara obtenida mediante la ruta de carga con \texttt{pickle} y la máscara obtenida mediante la ruta \textit{TorchScript}.}
	\label{fig:iou-venn}
\end{figure}
...
\subsection{Métricas temporales de rendimiento}
...
\begin{itemize}
	\item Tiempo de carga (\texttt{load\_ms}): Tiempo necesario para cargar el modelo desde disco.
	\item Tiempo de inferencia medio (\texttt{infer\_ms\_mean}): Media del tiempo de ejecución del \textit{forward} sobre una misma imagen, tras un calentamiento para estabilizar el rendimiento.
	\item Desviación estándar (\texttt{infer\_ms\_std}): Variabilidad del tiempo de inferencia entre repeticiones, empleado para detectar fluctuaciones del sistema.
\end{itemize}

\subsection{Métricas geométricas derivadas de la máscara}
...
\begin{itemize}
	\item Área segmentada (\texttt{area\_px}): Número de píxeles clasificados como traza, interpretado como la extensión total detectada.
	\item Longitud del esqueleto (\texttt{skeleton\_len\_px}): Longitud en píxeles del esqueleto de la máscara, utilizada como aproximación a la longitud de las sendas detectadas~\cite{zhang1984thinning}.
	\item Número de componentes conexas (\texttt{n\_components}): Cantidad de regiones desconectadas segmentadas, indicador de discontinuidades en la detección~\cite{gonzalez2008dip}.
	\item Grosor medio estimado (\texttt{mean\_thickness\_px}): Aproximación del grosor medio de la traza, calculada a partir de la relación entre área y longitud del esqueleto~\cite{zhang1984thinning}.
\end{itemize}
