...
Funciona de la siguiente manera: un modelo muy flexible, formado por varias capas de operaciones con parámetros y funciones no lineales, aprende a extraer representaciones jerárquicas de los datos ajustando sus parámetros para minimizar el error en los ejemplos de entrenamiento.
...
Esto se debe a que, a medida que la información o el gradiente pasan por muchas capas, pueden desvanecerse antes de alcanzar el inicio o el final de la red~\cite{huang2018densenet}.

\subsubsection{Predicción Densa (\textit{Dense prediction})}
...
El \textit{kernel} se desliza sobre la entrada con un \textit{stride} para producir la salida espacial~\cite{he2016resnet}. Mediante este proceso de \textit{downsampling} codifica la información de entrada en una representación de características de alto nivel submuestreada~\cite{ji2022automation, badrinarayanan2016segnet}.
...
En esta subsección se formaliza la evaluación experimental de un sistema de segmentación semántica, estableciendo un marco común para medir su rendimiento de manera reproducible y comparable~\cite{minaee2022segmentationSurvey,TahaHanbury2015}.

Para ello, se definen las métricas de evaluación empleadas, combinando medidas de solapamiento, rendimiento temporal y propiedades geométricas derivadas de la máscara con el objetivo de caracterizar tanto la equivalencia entre segmentaciones como su impacto estructural sobre las trazas detectadas~\cite{gonzalez2008dip,zhang1984thinning}.
...
Adicionalmente, la evaluación puede formularse en términos de la matriz de confusión, descomponiendo la comparación entre predicción y referencia en cuatro conteos básicos~\cite{sokolova2009systematic,powers2020}:

\begin{enumerate}
	\item \textbf{Verdadero positivo (TP)}
	 
	Corresponde a un píxel que pertenece a una traza en la referencia y que el modelo segmenta correctamente como traza. En términos prácticos, representa porciones de senda correctamente identificadas.
	
	\item \textbf{Falso positivo (FP)}
	  
	Se produce cuando el modelo segmenta como traza un píxel que en la referencia es fondo. En este problema suele interpretarse como ruido.
	
	\item \textbf{Falso negativo (FN)} 
	
	Un píxel que sí pertenece a una traza en la referencia pero el modelo lo clasifica como fondo. En la práctica representa omisiones, segmentos de traza no detectados.
	
	\item \textbf{Verdadero negativo (TN)}
	 
	Un píxel que es fondo en la referencia y que el modelo predice correctamente como fondo.
\end{enumerate}

En esta segmentación binaria, la clase positiva corresponde a píxeles pertenecientes a la traza de herbivoría, mientras que la clase negativa representa el suelo sin traza, que he denominado como fondo.

En la comparación realizada entre rutas de inferencia, las métricas de solapamiento se calculan entre la máscara obtenida al cargar el modelo mediante \texttt{pickle} y la adquirida al ejecutar la versión exportada a \textit{TorchScript}, sin emplear anotación externa~\cite{paszke2019pytorch}.

\subsubsection{Intersección sobre la Unión (IoU)}
La métrica de Intersección sobre la Unión (\textit{Intersection over Union}, IoU) mide el solapamiento relativo entre dos máscaras binarias. En esta comparativa se define como:
\begin{equation}
	\mathrm{IoU} = \frac{|M_{\text{pickle}} \cap M_{\text{torch}}|}{|M_{\text{pickle}} \cup M_{\text{torch}}|}
\end{equation}
donde \(M_{\text{pickle}}\) es la máscara obtenida por la ruta de inferencia que carga el modelo mediante \texttt{pickle} y \(M_{\text{torch}}\) la máscara generada por la ruta \textit{PyTorch/TorchScript}. Un valor cercano a uno indica una equivalencia espacial alta, mientras que valores menores reflejan discrepancias localizadas~\cite{minaee2022segmentationSurvey,Dimitrovski2024,TahaHanbury2015}.

Desde un punto de vista geométrico, esta comparación puede interpretarse como el solapamiento entre ambas máscaras, tal y como se ilustra en la Figura~\ref{fig:iou-venn}.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		\begin{scope}
			...
			\node at (-0.2,1.9) {$M_{\text{pickle}}$};
			\node at (1.8,1.9) {$M_{\text{torch}}$};
			\node at (0.7,0) {$\cap$};
		\end{scope}
	\end{tikzpicture}
	\caption{Interpretación geométrica de IoU como solapamiento entre la máscara obtenida mediante la ruta de carga con \texttt{pickle} y la máscara obtenida mediante la ruta \textit{TorchScript}.}
	\label{fig:iou-venn}
\end{figure}

\subsubsection{Coeficiente Dice}
El coeficiente \textit{Dice} es otra medida de solapamiento, especialmente utilizada en segmentación binaria, y en este contexto se calcula como:
\begin{equation}
	\mathrm{Dice} = \frac{2\,|M_{\text{pickle}} \cap M_{\text{torch}}|}{|M_{\text{pickle}}| + |M_{\text{torch}}|}
\end{equation}
Al igual que IoU, valores altos implican que ambas rutas de inferencia generan máscaras muy similares; no obstante, \textit{Dice} tiende a ser menos severo que IoU en casos de clases minoritarias~\cite{TahaHanbury2015,minaee2022segmentationSurvey}.

\subsubsection{Métricas temporales de rendimiento}
Para evaluar la eficiencia práctica de cada enfoque se midieron tiempos de carga y tiempos de inferencia por \textit{fold} en el banco de pruebas. En particular:
\begin{itemize}
	\item \textbf{Tiempo de carga (\texttt{load\_ms})} Tiempo necesario para cargar el modelo desde disco.
	\item \textbf{Tiempo de inferencia medio (\texttt{infer\_ms\_mean})} Media del tiempo de ejecución del \textit{forward} sobre una misma imagen, tras un calentamiento para estabilizar el rendimiento.
	\item \textbf{Desviación estándar (\texttt{infer\_ms\_std})} Variabilidad del tiempo de inferencia entre repeticiones, empleado para detectar fluctuaciones del sistema.
\end{itemize}

\subsubsection{Métricas geométricas derivadas de la máscara}
...
\begin{itemize}
	\item \textbf{Área segmentada (\texttt{area\_px})} Número de píxeles clasificados como traza, interpretado como la extensión total detectada.
	\item \textbf{Longitud del esqueleto (\texttt{skeleton\_len\_px})} Longitud en píxeles del esqueleto de la máscara, utilizada como aproximación a la longitud de las sendas detectadas~\cite{zhang1984thinning}.
	\item \textbf{Número de componentes conexas (\texttt{n\_components})} Cantidad de regiones desconectadas segmentadas, indicador de discontinuidades en la detección~\cite{gonzalez2008dip}.
	\item \textbf{Grosor medio estimado (\texttt{mean\_thickness\_px})} Aproximación del grosor medio de la traza, calculada a partir de la relación entre área y longitud del esqueleto~\cite{zhang1984thinning}.
\end{itemize}
