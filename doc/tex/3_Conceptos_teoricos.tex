...
En la comparación realizada entre rutas de inferencia, las métricas de solapamiento se calculan entre las máscaras generadas por \texttt{pickle} y por \textit{PyTorch/TorchScript}, sin emplear anotación externa~\cite{paszke2019pytorch}.
...
\end{equation}
donde \(M_{\text{pickle}}\) es la máscara generada por el modelo cargado con \texttt{pickle} y \(M_{\text{torch}}\) la máscara generada por la ruta \textit{PyTorch/TorchScript}. Un valor cercano a uno indica una equivalencia espacial alta, mientras que valores menores reflejan discrepancias localizadas~\cite{minaee2022segmentationSurvey,Dimitrovski2024,TahaHanbury2015}.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\end{scope}
	\end{tikzpicture}
	\caption{Interpretación geométrica de IoU como solapamiento entre las máscaras generadas por \texttt{pickle} y por \textit{PyTorch/TorchScript}.}
	\label{fig:iou-venn}
\end{figure}
