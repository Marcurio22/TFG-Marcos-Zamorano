...
\begin{itemize}
	\item Dispositivo con navegador web: se necesita un ordenador, portátil o dispositivo equivalente capaz de acceder a una aplicación web.
	
	\item Navegador actualizado: se recomienda emplear Google Chrome, Mozilla Firefox, Microsoft Edge, Safari u otro navegador moderno compatible con HTML5, CSS y JavaScript.
	
	\item JavaScript habilitado: la aplicación utiliza JavaScript para gestionar la interacción con la interfaz, el visor cartográfico, la colección de imágenes, los modales y diversos controles visuales.
	
	\item Conexión a internet: es necesaria para acceder al servidor donde se encuentre alojada la aplicación y, en su caso, para cargar los recursos cartográficos utilizados por el visor.
	
	\item Pantalla y dispositivo de entrada: se requiere una pantalla y un medio de interacción, como teclado y ratón o panel táctil, para utilizar formularios, botones, menús, mapas y galerías.
	
	\item Permiso de descarga en el navegador: debe permitirse la descarga de archivos para obtener imágenes procesadas, ficheros \texttt{ZIP} o resultados generados por la aplicación.
\end{itemize}

\section{Instalación}

El usuario final no necesita realizar ninguna instalación local para utilizar la aplicación desplegada. Únicamente debe disponer de un navegador web actualizado, conocer la URL o dirección IP del servidor y acceder a la dirección \texttt{http://10.168.168.34:8000}, siempre que los servicios estén levantados en el servidor mediante \texttt{Docker Compose}.

Debe tenerse en cuenta que esta dirección solo será accesible desde la red interna de la Universidad de Burgos. Por tanto, el acceso deberá realizarse estando conectado a la red de la UBU, a la red inalámbrica \textit{eduroam} o, en caso de conexión externa, mediante la VPN de la Universidad.

El funcionamiento de la interfaz se ha probado correctamente en los navegadores: Google Chrome y Mozilla Firefox.
...
La pantalla de inicio de sesión, mostrada en la Figura~\ref{fig:manual_login}, permite acceder a la aplicación mediante un nombre de usuario, o un correo, y una contraseña. En esta misma vista se ofrece también la posibilidad de continuar como visitante, opción pensada para usuarios que desean probar el funcionamiento básico de la herramienta sin crear una cuenta. Además, desde esta pantalla se puede acceder al formulario de registro mediante el enlace \textit{Registrarse}.
...
Durante la subida del modelo, la interfaz incorpora un estado intermedio para informar al administrador del avance de la operación. Como se muestra en la Figura~\ref{fig:manual_subiendo_modelo}, al seleccionar un archivo y comenzar la transferencia se añade una fila temporal de modelo en estado \textit{subiendo} y se muestra un bloque informativo con una barra de progreso. Este porcentaje refleja el avance real del envío desde el navegador al servidor. Asimismo, se muestra un aviso indicando que no debe abandonarse la página hasta que finalice la subida, ya que, si el usuario sale antes de tiempo, la transferencia puede cancelarse y el modelo no llegar a guardarse correctamente.

\begin{figure}[]
	\centering
	\includegraphics[width=\textwidth]{img/Subir_modelo}
	\caption[Progreso de subida de modelo]{Progreso de subida de un modelo desde el panel de administración.}
	\label{fig:manual_subiendo_modelo}
\end{figure}

Por otro lado, se ha incluido la opción \textit{Cancelar subida}, pensada para detener transferencias largas antes de que el archivo llegue completamente al servidor. Al pulsar este botón, el navegador aborta la petición de subida activa, elimina la fila temporal del modelo, oculta el bloque de progreso y muestra un mensaje de confirmación indicando que la operación se ha cancelado y que no se ha guardado ningún modelo. Esta cancelación afecta únicamente a la transferencia en curso; si el archivo ya se hubiera subido por completo y el servidor estuviera validando el modelo, dicha validación pertenecería a una fase posterior del flujo.

Tras guardar el archivo, el modelo se introduce inicialmente con estado de validación: pendiente y esta se inicia en segundo plano. A lo largo de este proceso, la aplicación comprueba que el modelo puede cargarse y ejecutar una inferencia de prueba. Si la validación finaliza correctamente, el modelo pasa a figurar como validado y podrá ser activado por el administrador. Por el contrario, si esta falla, el sistema informa del problema y el modelo no queda disponible para inferencia.
