...
\section{Análisis del \textit{notebook} de inferencia original}
...
Además, esta fase supuso un ejercicio importante de lectura y adaptación de código desarrollado por otra persona. No se trataba únicamente de ejecutar el \textit{notebook}, sino de comprender las decisiones tomadas en él, identificar dependencias implícitas, interpretar el flujo de datos y trasladar esa lógica a una arquitectura distinta. Este proceso resulta especialmente valioso desde un punto de vista profesional, ya que en un entorno empresarial será habitual trabajar sobre bases de código existentes, entender implementaciones ajenas y modificarlas de forma responsable para integrarlas en nuevos sistemas sin romper su comportamiento original.

\section{Comparativa de despliegue: modelos \textit{pickle} vs PyTorch}
...
\begin{itemize}
	\item Rendimiento: tiempo de carga del modelo por \textit{fold} (\texttt{load\_ms}) y tiempo de inferencia medio (\texttt{infer\_ms\_mean}) sobre la misma imagen.
	\item Equivalencia de salida: similitud entre máscaras binarias mediante IoU (\texttt{mask\_iou}) y Dice (\texttt{mask\_dice}).
\end{itemize}
...
\begin{table}[htbp]
	...
	\caption[Comparativa entre \textit{pickle} y TorchScript]{Comparativa por \textit{fold} entre carga e inferencia usando modelos serializados con \texttt{pickle} y exportados a TorchScript.}
...
\section{Procesos asíncronos en segundo plano}
...
Como resultado del proceso de revisión, el proyecto alcanzó un \textit{Quality Gate} superado en SonarCloud, con cobertura superior al umbral exigido y duplicidad dentro de los límites establecidos.

\section{Internacionalización de la interfaz}
...
\section{Protección frente a ataques CSRF}
...
Además de la protección CSRF, también se ha tenido en cuenta la seguridad en el acceso a recursos asociados al usuario, especialmente en el caso de las imágenes de perfil. Estas imágenes se sirven mediante rutas internas controladas, normalizando las rutas relativas y evitando que una URL manipulada pueda acceder a ficheros fuera del directorio permitido. De este modo, la aplicación no expone directamente rutas arbitrarias del sistema de ficheros y mantiene el acceso a estos recursos dentro de los límites previstos.

\section{Despliegue contenerizado con Docker y Gunicorn}
...
\section{Compatibilidad de PyTorch y CUDA en el despliegue}
