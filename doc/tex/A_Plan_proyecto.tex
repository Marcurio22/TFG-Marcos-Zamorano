...
\begin{itemize}
	...
	\item Incorporar Selenium, Docker, Gunicorn y SonarCloud en el apartado de herramientas.
	\item Redactar todo lo referente a Gunicorn y despliegue que todavía faltaba.
	\item Arreglar las tablas de anexos.
	\item Corregir problemas de SonarCloud:
	\begin{itemize}
...
\subsection{Sprint 22 - Cierre del TFG}
El \textit{Sprint 22} correspondió al cierre final del TFG y abarcó del 8 al 10 de junio, aunque en la práctica terminé el trabajo durante la madrugada del 11 de junio, aproximadamente a la 1:10~a.m. Para este \textit{sprint} estimé una dedicación de 27~horas, y finalmente el tiempo empleado fue muy similar, manteniéndose en torno a esa cifra.

Al tratarse de un \textit{sprint} de apenas tres días, no he considerado necesario incluir un \textit{burndown report} específico.

Los objetivos definidos para este \textit{sprint} fueron los siguientes:
\begin{itemize}
	\item Cambiar imágenes para satisfacer derechos de autor.
	\item Aplicar cambios sugeridos en la memoria.
	\item Actualizar el fichero \texttt{README.md}.
	\item Arreglar repetición de código detectada por SonarCloud.
	\item Arreglar problemas de \textit{responsiveness}.
	\item Redactar lo relativo al certificado SSL como línea futura.
	\item Arreglar el diagrama de secuencia.
	\item Redactar nuevos apartados de aspectos relevantes.
	\item Revisar imágenes de memoria y anexos que ya no se utilizasen.
	\item Aplicar cambios sugeridos en anexos.
	\item Realizar la última actualización de idiomas.
	\item Arreglar un problema de gestión del modelo en despliegue.
	\item Revisar el fichero \texttt{coverage.xml}.
	\item Revisar el fichero \texttt{pip freeze} y las dependencias finales.
	\item Revisar y actualizar la documentación completa del código.
\end{itemize}

En primer lugar, cambié las imágenes de fondo de la aplicación para satisfacer correctamente cuestiones de derechos de autor. En paralelo, apliqué los cambios sugeridos por el tutor en la memoria, corrigiendo redacción, estructura y distintos detalles pendientes para dejar el documento más pulido de cara a la entrega final.

A continuación, actualicé el fichero \texttt{README.md}, ajustándolo al estado final del proyecto y dejando más clara la información necesaria para su uso y ejecución. Después, corregí la repetición de código marcada por SonarCloud y arreglé problemas de \textit{responsiveness} en la aplicación, con el objetivo de mejorar la calidad del código y mantener una visualización adecuada en distintos tamaños de pantalla.

Más tarde, redacté el apartado relativo al certificado SSL dentro de las líneas futuras, ya que se trata de una mejora necesaria para un despliegue en condiciones reales, pero que no pude llegar a completar por falta de tiempo. También realicé la última revisión de los diagramas de secuencia, redacté nuevos apartados para los aspectos relevantes y revisé las imágenes de memoria y anexos que ya no se utilizaban.

El último día lo dediqué a cerrar los elementos más críticos que todavía quedaban pendientes. En primer lugar, terminé de aplicar los cambios sugeridos en los anexos y realicé las últimas traducciones de la aplicación. Después, cerré definitivamente el problema de gestión de modelos en despliegue, que había requerido más tiempo del esperado y que condicionó el cierre del \textit{sprint}. Debido a esta incidencia, no pude incorporar una funcionalidad que estaba prevista inicialmente: la recuperación de contraseña mediante correo electrónico, que quedó finalmente fuera del alcance del TFG.

Finalmente, revisé el fichero \texttt{coverage.xml} y el resultado de \texttt{pip freeze} para asegurar que el \texttt{requirements.txt} quedase alineado con las dependencias finales del proyecto. Como última tarea, actualicé la documentación de todo el código de la aplicación, dejando los módulos y funciones más importantes correctamente comentados antes de dar por cerrado el trabajo.
...
\begin{table}[]
	...
	\begin{tabularx}{\textwidth}{@{}
		...
		\midrule
		\textit{Python} & 3.13.0 & PSF \\
		\textit{Flask} & 3.1.2 & BSD-3 \\
		\textit{SQLAlchemy} & 2.0.49 & MIT \\
		\textit{SQLite} & 3.45.3 & Dominio público \\
		\textit{Jinja2} & 3.1.6 & BSD-3 \\
		\textit{Tailwind CSS} & 4.3.0 & MIT \\
		\textit{DaisyUI} & 5.5.23 & MIT \\
		\textit{Leaflet } & 1.9.4 & BSD-2 \\
		...
		\textit{SonarCloud Community Build} & 25.4 & LGPLv3 / SSAL v1.0 \\
		\bottomrule
	\end{tabularx}
